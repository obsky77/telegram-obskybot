...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        "Примеры: «Что в работе?», «Что по Сберу?», «Кто ведёт ПМФ?», «Что закрыли на этой неделе?»\n"
        "\n"
        "ДОБАВИТЬ ЗАДАЧУ\n"
        "/task — добавить задачу в спринт (уточню детали в диалоге)\n"
        "Примеры: «Добавь задачу: бриф по МТС, дедлайн 25 марта, приоритет П1»\n"
        ...
        "Примеры: «Где лежит презентация по ПМФ?», «Скинь ссылку на бриф по Сберу»,\n"
        "«Найди видео по МТС», «Где папка по Альфа ролику?», «Есть файлы по этому проекту?»\n"
        "Верну ссылку на папку и список всех файлов внутри.\n"
        "\n"
        "ФИДБЕК КОМАНДЕ\n"
        "«Передай команде: отличная работа по ПМФ!»\n"
        "«Напиши в журнал: клиент доволен роликом»\n"
        "Сообщение сохраняется в таблицу на лист Отзывы.\n"
        "\n"
        "ДАЙДЖЕСТ И ИНСАЙТЫ\n"
        ...
        "«Что по Сберу?» → «А кто ведёт?» → «Поставь им дедлайн на пятницу»\n"
