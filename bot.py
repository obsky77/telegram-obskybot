...
SHEET_URL = os.environ.get(
    "SHEET_URL",
    "https://docs.google.com/spreadsheets/d/12PGjDfUKdpo0oCPJXWIJXigEC78cIchhd2ySyfzJkc4/export?format=csv"
)
...
def build_system_prompt_with_sheet() -> str:
    ...
    return (
        SYSTEM_PROMPT
        + f"\n\n---\nАктуальные данные из рабочей таблицы (обновляется каждые 5 минут):\n\n{data}\n---"
    )
    ...
    try:
        response = anthropic.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": (
                    f"Вот данные из рабочей таблицы:\n\n{data}\n\n"
                    "Дай краткую аналитическую сводку: что в работе, что выполнено, "
                    "какие приоритеты, есть ли просроченные задачи или риски."
                )
            }]
        )
