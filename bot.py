...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "Вот что я умею. Всё работает и командой, и в свободном тексте — как удобнее.\n"
        "\n"
        "СТАТУС И ЗАДАЧИ\n"
        "/sprint — полный статус текущего спринта\n"
        "/hot — что горит прямо сейчас: П1 и ближайшие дедлайны\n"
        f"Примеры: «Что в работе?», «Что по {p1}?», «Кто ведёт {p2}?», «Что закрыли на этой неделе?»\n"
        "\n"
        "ДОБАВИТЬ ЗАДАЧУ\n"
        "/task — добавить задачу в спринт (уточню детали в диалоге)\n"
        f"Примеры: «Добавь задачу: бриф по {p1}, дедлайн 25 марта, приоритет П1»\n"
        "Для входящих: «Добавь во входящие: созвон с клиентом, от Маши»\n"
        "\n"
        "ИЗМЕНИТЬ ЗАДАЧУ\n"
        "«Поставь дедлайн [задача] на 1 апреля»\n"
        "«Измени приоритет [задача] на П1»\n"
        "«Добавь комментарий к [задача]: согласовано с клиентом»\n"
        "«Закрой задачу [название]»\n"
        "Задачу нахожу по частичному совпадению — точное название не нужно.\n"
        "\n"
        "СПРОСИТЬ МЕНЕДЖЕРА\n"
        "/ask [проект] [вопрос] — тегнуть менеджера проекта и задать вопрос\n"
        f"Примеры: «Спроси у менеджера по {p1} нужны ли правки», «Уточни у ответственного по {p2}»\n"
        "Если вопрос не указан — сформулирую из комментариев к задаче.\n"
        "\n"
        "ФАЙЛЫ И ПРЕЗЕНТАЦИИ\n"
        "/file [проект] — найти папку проекта в Google Drive и список файлов\n"
        f"Примеры: «Где лежит презентация по {p1}?», «Скинь файлы по {p3}?», «Есть файлы по этому проекту?»\n"
        ...
        "\n"
        "КАК ОБЩАТЬСЯ\n"
        "Можно спрашивать цепочкой — я помню контекст разговора:\n"
        f"«Что по {p1}?» → «А кто ведёт?» → «Поставь им дедлайн на пятницу»\n"
        "Если что-то непонятно — уточню сам, не нужно писать всё сразу.\n"
        "/clear — начать разговор с чистого листа\n"
        "\n"
        "В группе отвечаю на @упоминание или когда пишут «Огент»."
    )
