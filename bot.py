...
- Пиши как обычное сообщение в Telegram — никакой разметки вообще
- Запрещено: звёздочки (* **), решётки (#), нижние подчёркивания (_), тильды (~), вертикальные черты (|), таблицы
- Никаких заголовков секций — просто текст абзацами или список через обычный дефис (-)
- Короткие абзацы, пустая строка между смысловыми блоками
- Без пафоса, без «Итого:», без «Сводка:» — сразу по делу
- Эмодзи можно одно-два, только если реально уместно
- Короткая ирония или сухой юмор в конце — если уместно, не натужно
    ...
    try:
        ...
        resp = client.messages.create(
            ...
            messages=[{
                ...
                "content": (
                    f"Данные спринта:\n\n{data}\n\n"
                    f"Сегодня {today}. Перечисли все активные проекты спринта (без Done и cancel) "
                    "простым списком — каждый с новой строки через дефис. "
                    "Формат строки: название — дедлайн если есть — комментарий если есть. "
                    "Никакого деления по приоритетам, никаких секций и заголовков. "
                    "Просто список проектов. В конце одна строка: сколько всего задач в работе."
                )
            }]
        )
